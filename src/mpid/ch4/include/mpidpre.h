...
/* Netmod hook to release per-persistent-request state (e.g. cached MR).
 * Declared here (early) so MPID_Prequest_free_hook in ch4_request.h can call it
 * before the netmod's own headers are included. Defined by the active netmod;
 * only invoked for participating netmods (guarded at the call site). */
